  ...
  const packages = [
    ...
    },
    {
      ...
      features: [
        ...
        "Google Ads (up to ₹30K, 15% fee)",
        "Facebook/Instagram Ads",
        "20 social posts + 4 blog posts",
        "Email marketing (2 campaigns/month)",
        "Bi-weekly strategy calls",
        "Real-time analytics dashboard",
      ],
      expected: "150-300 qualified leads/month",
      bestFor: "Growing SMBs, ₹1Cr-5Cr revenue",
    ...
    },
    {
      ...
      features: [
        ...
        "Full social media (5 platforms)",
        "Content marketing (8 articles, 2 videos)",
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="mt-4 text-lg text-foreground/80">
            Choose your package. Add performance bonus for skin-in-the-game pricing.
          </p>
        </div>
        <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          {packages.map((pkg) => (
            <Card key={pkg.name} className={`flex flex-col transform hover:-translate-y-2 transition-transform duration-300 ${pkg.isHighlighted ? 'border-2 border-primary shadow-2xl relative' : 'shadow-lg'}`}>
              {pkg.isHighlighted && (
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-4 bg-primary text-primary-foreground px-4 py-1 rounded-full text-sm font-semibold">{pkg.badge}</div>
              )}
              <CardHeader className="pt-12">
                {!pkg.isHighlighted && <div className="text-sm font-semibold text-primary h-6">{pkg.badge}</div>}
                 {pkg.isHighlighted && <div className="h-6"></div>}
                <CardTitle className="text-3xl font-bold">{pkg.name}</CardTitle>
                <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-extrabold">{pkg.price}</span>
                    <span className="text-foreground/70">{pkg.billing}</span>
                </div>
              </CardHeader>
        ...
        <div className="mt-12 grid md:grid-cols-2 gap-8">
            <div className="border-2 border-primary border-dashed rounded-lg p-8 text-center md:text-left">
                ...
                <p className="mt-2 text-foreground/80">Add 10-15% of qualified lead value to any package. We only earn extra when you get results.</p>
            ...
            <div className="bg-secondary text-secondary-foreground rounded-lg p-8 text-center md:text-left">
                <h3 className="text-2xl font-bold flex items-center gap-2 justify-center md:justify-start">
                    <Shield className="text-accent h-6 w-6"/> 90-Day Money-Back Guarantee
                </h3>
